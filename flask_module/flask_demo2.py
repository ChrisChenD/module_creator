...
class plan(Flask_url):
    ...
    def save_plan(self, plan):
        plan_copy = self.data[plan]
        import pickle
        with open(f'plan_lib/{plan}.pkl', 'wb') as f:
            pickle.dump(plan_copy, f)
    def load_plan(self, plan):
        plan_copy = dict()
        import pickle
        try:
            with open(f'plan_lib/{plan}.pkl', 'rb') as f:
...
class Plan2:
    ...
    def get(self, plan):
        from plan2_module.plan2 import plan_router_maker
        if plan not in self.data:
            self.data[plan] = plan_router_maker(plan)
        return self.data[plan].call([{
            'method': 'to_dict',
        }
        ])
    def post(self, router_list, plan):
        self.data[plan].call(router_list)
